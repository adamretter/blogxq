...
<xsl:stylesheet xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns="http://www.w3.org/1999/xhtml" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xh="http://www.w3.org/1999/xhtml" xmlns:geo="http://www.w3.org/2003/01/geo/wgs84_pos#" xmlns:foaf="http://xmlns.com/foaf/0.1/" xmlns:ars="http://www.adamretter.org.uk" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:og="http://opengraphprotocol.org/schema/" xmlns:dcterms="http://purl.org/dc/terms/" version="2.0">
    <xsl:output doctype-public="-//W3C//DTD XHTML+RDFa 1.0//EN" doctype-system="http://www.w3.org/MarkUp/DTD/xhtml-rdfa-1.dtd" encoding="UTF-8" indent="yes" omit-xml-declaration="no" media-type="text/html" method="xhtml" version="1.0" exclude-result-prefixes="xs xh ars"/>
    ...
    <xsl:template match="ars:page" exclude-result-prefixes="xs xh ars">
        ...
        <html xml:lang="en">
            ...
            <head profile="http://dublincore.org/documents/2008/08/04/dc-html/">
                <xsl:element name="title">Adam Retter<xsl:if test="@title"> - <xsl:value-of select="@title"/>
                    </xsl:if>
                </xsl:element>
                <xsl:element name="meta">
                    <xsl:attribute name="name">DC.title</xsl:attribute>
                    <xsl:attribute name="content">Adam Retter<xsl:if test="@title"> - <xsl:value-of select="@title"/>
                        </xsl:if>
                    </xsl:attribute>
                </xsl:element>
                <xsl:element name="meta">
                ...
                <link rel="stylesheet" type="text/css" href="{$pathToRoot}resources/css/default.css"/>
                <link rel="icon" type="image/gif" href="{$pathToRoot}resources/images/db1.gif"/>
            ...
            <body>
                ...
                <div id="outer">
                    ...
                    <div id="menu">
                        <ul>
                            <li>
                                <a href="{$pathToRoot}home.xml" title="Home page" class="{if(@ars:name eq 'Home')then 'active' else 'unactive'}">Home</a>
                            </li>
                            <li>
                                <a href="{$pathToRoot}blog.xql" title="Blog" class="{if(@ars:name eq 'Blog')then 'active' else 'unactive'}">Blog</a>
                            </li>
                            <li>
                                <a href="{$pathToRoot}presentations.xml" title="Presentations" class="{if(@ars:name eq 'Presentations')then 'active' else 'unactive'}">Presentations</a>
                            </li>
                            <xsl:apply-templates select="ars:menu-item"/>
                        </ul>
                    </div>
                    <div id="menubottom"/>
                    <xsl:apply-templates select="@*[local-name(.) ne 'title']|node()[node-name(.) ne QName('http://www.adamretter.org.uk','head')]"/>
                    <div id="footer">
                        <div class="left">
                            ...
                            <a href="http://jigsaw.w3.org/css-validator/validator?uri=http://www.exquery.org/styles.css">
                                <img src="{$pathToRoot}resources/images/valid-css.gif" alt="Valid CSS!" style="border: 0;"/>
                            </a>
                            <a href="http://dublincore.org/" title="Dublin Core Meadata Initiative">
                                <img src="{$pathToRoot}resources/images/dcuh_88x31.gif" alt="Dublin Core Used Here" style="border: 0;"/>
                            </a>
                            <a href="http://www.ogp.me/" title="Open Graph Protocol">
                                <img src="{$pathToRoot}resources/images/ogp_88x31.png" alt="Open Graph Protocol Used Here" style="border: 0;"/>
                            </a>
                            <br/> This <span href="http://purl.org/dc/dcmitype/Text" rel="dc-legacy:type">work</span> is licensed under a <a rel="license" href="http://creativecommons.org/licenses/by-nc-nd/3.0/">Creative
                                Commons Attribution-NonCommercial-NoDerivs 3.0 Unported License</a>. </div>
                        <div class="right">
                            <div>
                                <a href="http://www.openindiana.org" title="Open Indiana">Open
                                    Indiana</a> hosting by: <a href="http://www.entic.net">Entic.net</a>
                            </div>
    ...
    <xsl:template match="xh:img | xh:a | xh:form">
        <xsl:element name="{local-name()}">
            <xsl:for-each select="@*">
                <xsl:choose>
                    <xsl:when test="local-name(.) = ('src','href','action') and not(starts-with(., 'http://'))">
