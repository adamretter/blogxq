...
declare function kwic:get-summary($root as node(), $node as element(exist:match), 
	...
	return
		if (not($table)) then
			<xh:p>
				<xh:span class="previous">{$prevTrunc}</xh:span>
				{
					if ($config/@link) then
						<xh:a class="hi" href="{$config/@link}">{ $node/text() }</xh:a>
					else
						<xh:span class="hi">{ $node/text() }</xh:span>
				}
				<xh:span class="following">{$followingTrunc}</xh:span>
			</xh:p>
		else
			<xh:tr>
				<xh:td class="previous">{$prevTrunc}</xh:td>
				<xh:td class="hi">
				{
					if ($config/@link) then
						<xh:a href="{$config/@link}">{$node/text()}</xh:a>
					else
						$node/text()
				}
				</xh:td>
				<xh:td class="following">{$followingTrunc}</xh:td>
			</xh:tr>
};
