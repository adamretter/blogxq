...
import module namespace config = "http://www.adamretter.org.uk/config" at "config.xqm";
(: import module namespace recaptcha = "http://www.exist-db.org/xquery/util/recapture" at "xmldb:exist:///db/util/recapture.xqm"; :)
import module namespace asirra = "http://asirra.com/xquery/api" at "asirra.xqm";
import module namespace akismet = "http://akismet.com/xquery/api" at "akismet.xqm";

declare variable $local:akismet-api-key := "20139c84c379f20fb691267cee79e557";

declare option exist:serialize "method=xhtml";

declare variable $local:app-collection := '/db/apps/blogxq';
declare variable $local:entries-collection := concat($local:app-collection, '/entries');
declare variable $local:comments-collection := concat($local:entries-collection, '/comments');
declare variable $local:site-xslt := doc("/db/apps/blogxq/resources/stylesheets/site.xslt");
declare variable $local:entry-xslt := doc("/db/apps/blogxq/resources/stylesheets/entry.xslt");
declare variable $local:root-path := '/exist/apps/blogxq/';
declare variable $local:resources-path := concat($local:root-path, '/resources/');
...
{
    if(xmldb:login($local:entries-collection, $username, $password, true()))then
    (
...
declare function local:mark-comment-as-spam($comment-id as xs:string) as xs:string {
    let $comment := fn:collection($local:comments-collection)/blog:comment[@id eq $comment-id],
    $entry := fn:collection($local:entries-collection)/blog:entry[@id eq $comment/@entry-id] return
...
declare function local:remove-comment-as-spam($comment-id as xs:string) {
    ...
    let $entry-id := local:mark-comment-as-spam($comment-id) return
        ...
        (: delete the comment :)
        let $comment-uri := fn:document-uri(fn:root(fn:collection($local:entries-collection)/blog:comment[@id eq $comment-id])) return
...
{
    xmldb:store($local:comments-collection, fn:concat($comment/@id, ".xml"), $comment)
};
...
declare function local:edit-page($entry-uri as xs:anyURI, $resources-path as xs:string?) as element(ars:page)
{
    ...
    let $entry := doc(concat($local:app-collection, $entry-uri))/blog:entry return
        ...
        <ars:page ars:name="Blog">
                ...
                <ars:head>
                    <xh:script type="text/javascript" src="{$resources-path}resources/scripts/ckeditor/ckeditor.js"></xh:script>
                    <xh:script type="text/javascript" src="{$resources-path}resources/scripts/jquery-1.4.2.min.js"></xh:script>                    
                </ars:head>
                <ars:content>
                    <ars:main>
                        <ars:title>Blog</ars:title>
...
declare function local:save-page($entry-uri as xs:anyURI, $resources-path as xs:string?) as element(ars:page)
{
    ...
    let $page := local:page(util:function(xs:QName("local:entry-filter"), 1), $resources-path) return
        transform:stream-transform($page, $local:site-xslt, ())
};
...
declare function local:get-comments-for-entries($entries as element(blog:entry)+) as element(blog:comment)* {
    collection($local:comments-collection)/blog:comment[@entry-id = $entries/@id]
};
...
declare function local:page($filter-function, $resources-path as xs:string?) as element(ars:page)
{
    ...
    $filtered-entries := $blog-entries (:util:call($filter-function, $blog-entries):),
            ...
            <ars:content>
                <ars:main>
                    ...
                    <xh:a href="?atom" title="Atom Feed" alt="Atom Feed">
                        <xh:img src="resources/images/atom.jpg" style="border: 0" alt="Atom Feed" title="Atom Feed"/>
                    </xh:a>
                </ars:main>
                <ars:additionals>
                {
                    let $show-comments := ($filtered-entries-count eq 1) return
                        ...
                        (   
                            let $entry-transform := $local:entry-xslt return
...
{
    xs:anyURI(substring-after(string($db-uri), 'blogxq/'))
};
...
{
    collection($local:entries-collection)/blog:entry[@status = $status]
};
...
let $is-authenticated := not(empty(session:get-attribute('_eXist_xmldb_user'))) return
    ...
    (
        let $page :=  if(request:get-parameter("entry",()))then
            ...
            (
                local:page(util:function(xs:QName("local:no-filter"), 1), $local:root-path)
            ) return
                ...
                (
                    (:
                    transform:transform($page, $local:site-xslt), ())
                    :)
                    transform:stream-transform($page, $local:site-xslt, ())
                )
    )